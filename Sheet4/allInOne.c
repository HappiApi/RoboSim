...
#define LEFT 0
#define RIGHT 1
#define encToCentimetres (9.55 * 3.141592 / 360)
#define widthOfRobot 22.5   // In cm
...
int leftEnc; int rightEnc; int leftEncPrev; int rightEncPrev; double xCurrent; double yCurrent;
...
void calcPosition(int leftEnc, int rightEnc) 
{
  ...
  {
    angleTurned = (double)( leftEnc - rightEnc ) * encToCentimetres / widthOfRobot ;
...
int main() {
  ...
  while (1) {
    ...
    if (get_front_ir_dist(RIGHT) < 25) {
      ...
      get_motor_encoders(&leftEnc,&rightEnc);
      calcPosition(leftEnc - leftEncPrev,rightEnc - rightEncPrev);
      leftEncPrev = leftEnc;
